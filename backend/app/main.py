...
# ── ML models (NEW) ──────────────────────────────────────────────────────────
from backend.app.models.ml import (                                   # ← NEW
    QuestionDifficulty,
...
from backend.app.routers import settings as settings_router
from backend.app.routers import notifications, leaderboard, admin
from backend.app.routers import ml as ml_router                       # ← NEW
...
async def lifespan(app: FastAPI):
    ...
    try:
        check_db_connection()
        Base.metadata.create_all(bind=engine)
        logger.info("Database tables verified (including ML tables). Startup complete.")
    except Exception as exc:
        logger.critical(f"Startup failed: {exc}")
        raise
    yield
...
app = FastAPI(
    title=settings.APP_NAME,
    version=settings.APP_VERSION,
    description="Curio — Cloud Quiz Platform API with ML Features",
    docs_url="/docs",
    redoc_url="/redoc",
    openapi_url="/openapi.json",
    lifespan=lifespan,
