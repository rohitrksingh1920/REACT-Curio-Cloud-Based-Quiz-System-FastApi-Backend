...
# # from backend.app.models.quiz import (
# #     Quiz, Question, QuestionOption,
# #     QuizEnrollment, QuizStatus,
# # )
# # from backend.app.models.user import User, UserRole
# # from backend.app.schemas.misc import (
# #     AttemptResult, EnrollRequest, EnrollResponse,
# # )
# # from backend.app.schemas.quiz import (
# #     QuizCreate, QuizDetail, QuizPublic, QuizSummary, QuizUpdate,
# #     QuestionOut, QuestionOutPublic, OptionOut, OptionOutPublic,
...
# # # ML imports — wrapped so missing migration never breaks core quiz flow
# # try:
# #     from backend.app.core.ml_engine import (
# #         build_topic_profile,
# #         compute_smart_leaderboard_score,
...
# # # ── BUG 2 FIX: single merged submit schema ────────────────────────────────────

# # class QuestionTimingItem(BaseModel):
# #     question_id:  int
# #     time_taken:   float   # seconds spent on this question
# #     option_index: int     # 0-based index of selected option (for pattern detection)
# #     is_correct:   bool    # backend will override this, but schema needs the field


# # class SubmitPayload(BaseModel):
# #     """
# #     Single JSON body for quiz submission.
# #     answers   — required: list of question_id + selected_option_id
# #     timings   — optional: per-question timing for ML adaptive engine + cheating detection
# #                 If absent, ML features are skipped; core scoring still works.
# #     """
# #     answers: List[dict]                         # [{"question_id": int, "selected_option_id": int}]
# #     timings: Optional[List[QuestionTimingItem]] = None   # ← Optional, never "required"
...
# #             raise HTTPException(status_code=422, detail="Quiz title cannot be empty.")
# #         quiz.title = payload.title.strip()
# #     if payload.category is not None:
# #         quiz.category = payload.category
# #     if payload.duration_mins is not None:
...
# # # ── Submit quiz (BUG 2 FIX) ───────────────────────────────────────────────────

# # @router.post("/{quiz_id}/submit", response_model=AttemptResult)
# # def submit_quiz(
# #     quiz_id: int,
# #     payload: SubmitPayload,           # ← single schema, timings is Optional inside
# #     current_user: User = Depends(get_current_user),
# #     db: Session = Depends(get_db),
# # ):
# #     """
...
# #     BUG 2 FIX: timings is now an Optional field inside SubmitPayload,
# #     NOT a separate Body() parameter. This means the frontend can send:
# #       { "answers": [...] }                    ← works (no timings, ML skipped)
# #       { "answers": [...], "timings": [...] }  ← works (ML features active)

# #     Previously FastAPI demanded timings as a separate JSON key and returned
# #     422 "Field required" when it was absent.
# #     """
# #     quiz = db.query(Quiz).filter(Quiz.id == quiz_id).first()
# #     if not quiz:
# #         raise HTTPException(status_code=404, detail="Quiz not found.")
...
# #         is_correct = correct_map.get(qid) == oid
# #         if is_correct:
# #             correct_count += 1
# #         db.add(AttemptAnswer(
# #             attempt_id=attempt.id,
...
# #             try:
# #                 update_question_difficulty_stats(
# #                     db=db, question_id=qid, is_correct=is_correct
# #                 )
# #             except Exception:
# #                 pass
...
# #     # ML: persist timings (optional — only if sent by frontend)
# #     if ML_ENABLED and payload.timings:
# #         try:
# #             timing_map = {t.question_id: t for t in payload.timings}
# #             for ans in payload.answers:
# #                 qid = ans.get("question_id")
# #                 t   = timing_map.get(qid)
# #                 if t:
# #                     db.add(AttemptQuestionTiming(
# #                         attempt_id=attempt.id,
# #                         question_id=qid,
# #                         time_taken_seconds=t.time_taken,
# #                         is_correct=(correct_map.get(qid) == ans.get("selected_option_id")),
# #                     ))
# #         except Exception:
# #             pass
...
# #     # ML: rebuild topic profile + smart leaderboard
# #     if ML_ENABLED:
# #         try:
# #             build_topic_profile(db=db, user_id=current_user.id)
# #         except Exception:
...
# #     # ML: cheating detection (only when timings are present)
# #     if ML_ENABLED and payload.timings:
# #         try:
# #             timing_dicts = [
# #                 {
...
# #                     "time_taken":   t.time_taken,
# #                     "option_index": t.option_index,
# #                     "is_correct":   correct_map.get(t.question_id) == next(
# #                         (a.get("selected_option_id") for a in payload.answers
# #                          if a.get("question_id") == t.question_id),
# #                         None,
# #                     ),
# #                 }
...
# )
# from backend.app.models.user import User, UserRole
# from backend.app.schemas.misc import AttemptResult, EnrollRequest, EnrollResponse
# from backend.app.schemas.quiz import (
#     QuizCreate, QuizDetail, QuizPublic, QuizSummary, QuizUpdate,
#     QuestionOut, QuestionOutPublic, OptionOut, OptionOutPublic,
# )

# # ── ML imports — all wrapped so a missing migration NEVER breaks quiz flow ────
# try:
#     from backend.app.core.ml_engine import (
#         build_topic_profile,
#         compute_smart_leaderboard_score,
...
#         update_question_difficulty_stats,
#     )
#     from backend.app.models.ml import AttemptQuestionTiming, QuestionDifficulty, DifficultyLevel
#     ML_ENABLED = True
# except Exception:
#     ML_ENABLED = False
...
# # ── Submit payload schema (FIX: timings is Optional, not required) ────────────
...
# class SubmitPayload(BaseModel):
#     """
#     Single JSON body — timings is Optional so:
#       { "answers": [...] }                    → works, ML skipped
#       { "answers": [...], "timings": [...] }  → works, ML active
#     """
#     answers: List[dict]
#     timings: Optional[List[QuestionTimingItem]] = None
...
#     quiz_dt = _quiz_ist_datetime(quiz)
#     if quiz_dt and now_ist() < quiz_dt:
#         ist_str = quiz_dt.strftime("%d %b %Y, %H:%M IST")
#         raise HTTPException(
#             status_code=403,
#             detail=f"This quiz is not yet available. It starts at {ist_str}.",
...
# # ── Submit quiz — THE KEY ML INTEGRATION POINT ───────────────────────────────

# @router.post("/{quiz_id}/submit", response_model=AttemptResult)
# def submit_quiz(
#     quiz_id: int,
#     payload: SubmitPayload,       # ← single schema, timings optional inside
#     current_user: User = Depends(get_current_user),
#     db: Session = Depends(get_db),
# ):
#     """
...
#     ML HOOKS (all in try/except — never break core scoring):
#       1. update_question_difficulty_stats() per answer
#       2. Persist per-question timings to attempt_question_timing
#       3. build_topic_profile() — updates user_topic_profile
#       4. compute_smart_leaderboard_score() — updates smart_leaderboard_score
#       5. detect_suspicious_behavior() — creates cheating_flags if needed
#     """
#     quiz = db.query(Quiz).filter(Quiz.id == quiz_id).first()
#     if not quiz:
#         raise HTTPException(status_code=404, detail="Quiz not found.")
...
#     # Score answers + ML hook 1: update difficulty stats
#     correct_count = 0
#     for ans in payload.answers:
#         qid = ans.get("question_id")
#         oid = ans.get("selected_option_id")
...
#             is_correct=is_correct,
#         ))
#         # ── ML hook 1: update per-question difficulty stats ───────────────────
#         if ML_ENABLED:
#             try:
#                 update_question_difficulty_stats(db=db, question_id=qid, is_correct=is_correct)
#             except Exception:
#                 pass
...
#     # ── ML hook 2: persist question timings ───────────────────────────────────
#     if ML_ENABLED and payload.timings:
#         try:
#             timing_map = {t.question_id: t for t in payload.timings}
#             for ans in payload.answers:
...
#     # ── ML hook 3: rebuild topic profile ─────────────────────────────────────
#     if ML_ENABLED:
#         try:
#             build_topic_profile(db=db, user_id=current_user.id)
#         except Exception:
...
#     # ── ML hook 4: update smart leaderboard score ─────────────────────────────
#     if ML_ENABLED:
#         try:
#             compute_smart_leaderboard_score(
#                 db=db, user_id=current_user.id, quiz_id=quiz_id
...
#     # ── ML hook 5: cheating detection (only when timings present) ─────────────
#     if ML_ENABLED and payload.timings:
#         try:
#             timing_dicts = [
#                 {
