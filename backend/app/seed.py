...
seed.py  (IST edition)
----------------------
Historical attempt timestamps now use now_ist() instead of datetime.now(timezone.utc).
"""
...
def seed():
    ...
    try:
        ...
        quiz_data = [
            {
                ...
                "total_points": 100,
                "scheduled_date": date(2026, 6, 20),
                "scheduled_time": time(10, 0),   # 10:00 AM IST
                "status": QuizStatus.upcoming,
                "questions": [
                    {
                        "text": "What does the GIL stand for in Python?",
            ...
            },
            {
                ...
                "total_points": 100,
                "scheduled_date": date(2026, 6, 22),
                "scheduled_time": time(14, 0),   # 2:00 PM IST
                "status": QuizStatus.upcoming,
                "questions": [
                    {
                        "text": "In which year did World War II begin?",
        ...
        assignment_map = {
            python_quiz.id: [students[0], students[1], students[2]],
            wwii_quiz.id:   [students[0], students[1], students[3], students[4]],
            dsa_quiz.id:    students,
            math_quiz.id:   [students[2], students[3], students[4]],
        }
        ...
        # ── Historical completed attempts ─────────────────────────────────────
        # Timestamps are generated in IST so analytics display correctly.
        historical = [
            (QuizCategory.computer_science, 75, 14),
            (QuizCategory.mathematics,      80, 13),
            (QuizCategory.history,          72, 12),
            ...
            (QuizCategory.computer_science, 88,  5),
            (QuizCategory.mathematics,      84,  4),
            (QuizCategory.computer_science, 100, 1),
        ]
        ...
        for cat, score_pct, days_ago in historical:
            ...
            # now_ist() gives the current IST time; subtract days for history
            completed_dt = now_ist() - timedelta(days=days_ago)  # ← IST timestamp
            db.add(QuizAttempt(
                user_id=rohit.id,
                quiz_id=hist_quiz.id,
                score=float(score_pct),
        ...
        print("✅  Seeding complete! (timestamps in IST)")
        print("\n  Demo accounts:")
        print("  ┌──────────────────────────────────────────────────────────────┐")
        print("  │  Admin   │ admin@projexi.com              │ admin1234        │")
        print("  │  Teacher │ rohitrk.singh1920@gmail.com    │ rohit1234        │")
