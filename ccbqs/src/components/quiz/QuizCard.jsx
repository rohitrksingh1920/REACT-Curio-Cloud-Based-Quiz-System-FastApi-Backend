...
export default function QuizCard({ quiz, onDelete, showDelete = false }) {
  ...
  // BUG 1 FIX: formatTimeIST returns null for null/invalid values
  const formattedDate = quiz.scheduled_date ? formatDateIST(quiz.scheduled_date) : null
  const formattedTime = quiz.scheduled_time ? formatTimeIST(quiz.scheduled_time) : null
  ...
  return (
    <div className="card" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div className="quiz-card-inner">
        ...
        <div className="quiz-card-meta">
          {/* Only show date row when there IS a scheduled date */}
          {formattedDate && (
            <div className="quiz-card-meta-item">
              <i className="ri-calendar-line" />
          ...
          {/* Only show time row when there IS a valid scheduled time — Bug 1 fix */}
          {formattedTime && (
            <div className="quiz-card-meta-item">
              <i className="ri-time-line" />
              <span>{formattedTime} IST · {quiz.duration_mins} min</span>
            </div>
          )}

          {/* Always show duration row — shows instead of time when no schedule */}
          {!formattedTime && (
            <div className="quiz-card-meta-item">
              <i className="ri-timer-line" />
              <span>{quiz.duration_mins} min · {quiz.total_points} pts</span>
            </div>
          )}
