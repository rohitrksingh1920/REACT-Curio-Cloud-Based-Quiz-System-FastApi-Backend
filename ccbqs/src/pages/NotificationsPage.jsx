...
import { timeAgoIST } from '../utils/dateUtils'   // ← IST time-ago
    ...
    <div className={`notif-card${notif.is_read ? '' : ' unread'}`}>
      ...
      <div className="notif-body">
        ...
        <div className="notif-time">
          ...
          {/* timeAgoIST computes diff from IST now */}
          {timeAgoIST(notif.created_at)}
        </div>
      </div>
          ...
          <div className="filter-bar" style={{ margin: 0 }}>
            ...
            <button className={`filter-chip${filter === 'unread' ? ' active' : ''}`} onClick={() => setFilter('unread')}>
              Unread {unreadCount > 0 && (
                <span style={{ background: 'var(--primary)', color: '#fff', borderRadius: 99, fontSize: 10, fontWeight: 700, padding: '1px 6px', marginLeft: 4 }}>
                  {unreadCount}
                </span>
              )}
            </button>
