...
import { LeaderboardAPI, QuizAPI } from '../api'
import { useAuth } from '../context/AuthContext'
import { formatCompactIST } from '../utils/dateUtils'   // ← IST compact datetime
import './LeaderboardPage.css'
...
function Avatar({ name, picture, size = 38 }) {
  ...
  return (
    <div style={{ width: size, height: size, borderRadius: '50%', background: 'var(--primary)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: size * 0.35, fontWeight: 700, fontFamily: 'var(--font-display)', flexShrink: 0 }}>
      {initials}
    </div>
  )
}
...
function TopThreeCard({ entry }) {
  ...
  return (
    <div className={`lb-top-card ${entry.is_current_user ? 'lb-me' : ''}`}>
      <div className="lb-top-medal" style={{ color: colors[entry.rank - 1] }}>{medals[entry.rank - 1]}</div>
      <Avatar name={entry.full_name} picture={entry.profile_picture} size={sizes[entry.rank - 1]} />
      <div className="lb-top-name">{entry.full_name}</div>
      <div className="lb-top-score">{entry.score_pct}%</div>
      <div className="lb-top-detail">{entry.correct_count}/{entry.total_questions} correct</div>
      {entry.is_current_user && (
        <span style={{ background: 'var(--primary)', color: '#fff', fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 99, marginTop: 4 }}>YOU</span>
      )}
    </div>
  )
}
    ...
    <AppShell title="Leaderboard" subtitle="See how everyone performed">
      <div className="card" style={{ padding: 20, marginBottom: 24 }}>
        <div className="flex items-center gap-3" style={{ flexWrap: 'wrap' }}>
          ...
          <select value={selectedQuiz} onChange={e => { setSelectedQuiz(e.target.value); navigate(`/leaderboard/${e.target.value}`) }} style={{ maxWidth: 360 }}>
            <option value="">— Choose a quiz —</option>
      ...
      {!selectedQuiz ? (
        <div className="card"><div className="empty-state"><i className="ri-trophy-line" /><h3>Select a quiz</h3><p>Pick a quiz above to view its leaderboard.</p></div></div>
      ) : loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {[1,2,3].map(i => <div key={i} className="skeleton" style={{ height: i === 1 ? 60 : i === 2 ? 180 : 260, borderRadius: 'var(--radius-md)' }} />)}
        </div>
      ) : error ? (
        <div className="error-banner"><i className="ri-error-warning-line" /> {error}</div>
      ) : data?.total_participants === 0 ? (
        <div className="card"><div className="empty-state"><i className="ri-user-search-line" /><h3>No attempts yet</h3><p>No one has completed this quiz yet. Be the first!</p></div></div>
      ) : (
        <>
          <div className="lb-info-bar card mb-6">
            <div className="lb-info-item"><i className="ri-survey-line" /><span>{data.quiz_title}</span></div>
            <div className="lb-info-item"><i className="ri-group-line" /><span>{data.total_participants} participants</span></div>
            {data.current_user_rank && (
              <div className="lb-info-item" style={{ color: 'var(--primary)', fontWeight: 700 }}>
                <i className="ri-user-star-line" /><span>Your rank: #{data.current_user_rank}</span>
              </div>
            )}
          </div>
                ...
                <table>
                  <thead>
                    <tr>
                      ...
                      <th>Score</th>
                      <th>Correct</th>
                      {/* Column header clarifies IST */}
                      <th>Completed (IST)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {rest.map(entry => (
                      <tr key={entry.user_id} className={entry.is_current_user ? 'lb-me-row' : ''}>
                        <td><span className="lb-rank-badge">#{entry.rank}</span></td>
                        <td>
                          <div className="flex items-center gap-3">
                            <Avatar name={entry.full_name} picture={entry.profile_picture} size={32} />
                            <div>
                              <div style={{ fontWeight: 600, fontSize: 14 }}>{entry.full_name}</div>
                              {entry.is_current_user && <span style={{ fontSize: 10, color: 'var(--primary)', fontWeight: 700 }}>YOU</span>}
                            </div>
                          </div>
                        </td>
                        <td>
                          <span style={{ fontWeight: 700, fontSize: 15, color: entry.score_pct >= 60 ? 'var(--success)' : 'var(--danger)' }}>
                            {entry.score_pct}%
                          </span>
                        </td>
                        <td style={{ color: 'var(--text-2)', fontSize: 13 }}>{entry.correct_count}/{entry.total_questions}</td>
                        {/* formatCompactIST converts UTC ISO → IST display */}
                        <td style={{ color: 'var(--text-3)', fontSize: 12 }}>{formatCompactIST(entry.completed_at)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
