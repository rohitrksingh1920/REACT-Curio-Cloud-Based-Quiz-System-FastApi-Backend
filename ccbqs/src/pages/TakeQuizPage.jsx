...
function QuizError({ message, onBack }) {
  const isNotAvailable =
    message?.toLowerCase().includes('not yet available') ||
    message?.toLowerCase().includes('starts at')
  ...
  return (
    <div className="quiz-result-page">
      <div className="quiz-result-card">
        ...
        <p style={{ color: 'var(--text-2)', marginBottom: 32, fontSize: 14 }}>
          {passed ? 'Great job! You passed the quiz.' : "Keep practising — you'll get it next time!"}
        </p>
...
export default function TakeQuizPage() {
  ...
  // Timing tracking for ML cheating detection + adaptive engine
  const questionStartRef = useRef(Date.now())
  const timingsRef       = useRef({})     // { questionId: { time_taken, option_index } }
  const timerRef         = useRef(null)
  ...
  function recordTiming(questionId, optionIndex = -1) {
    const elapsed = Math.max(0.5, (Date.now() - questionStartRef.current) / 1000)
    timingsRef.current[questionId] = {
      time_taken:   elapsed,
  ...
  function handleSelectAnswer(questionId, optionId, optionIndex) {
    recordTiming(questionId, optionIndex)
    setAnswers(prev => ({ ...prev, [questionId]: optionId }))
  }

  function handleNavigate(newIdx) {
    // Record timing for current question before navigating away
    const questions = quiz?.questions || []
    const currentQ  = questions[current]
    if (currentQ) {
      const selOptIdx = (currentQ.options || []).findIndex(o => o.id === answers[currentQ.id])
      recordTiming(currentQ.id, selOptIdx)
    }
    setCurrent(newIdx)
    questionStartRef.current = Date.now()
  }
  ...
  const handleSubmit = useCallback(async (auto = false) => {
    ...
    // Record timing for the current question before submitting
    const currentQ = questions[current]
    if (currentQ) {
      const selOptIdx = (currentQ.options || []).findIndex(o => o.id === answers[currentQ.id])
      recordTiming(currentQ.id, selOptIdx)
    ...
    // Build timings array for ML (optional)
    const timingsPayload = questions
      .filter(q => timingsRef.current[q.id])
      .map(q => {
        const t       = timingsRef.current[q.id]
        const selIdx  = (q.options || []).findIndex(o => o.id === answers[q.id])
        return {
          question_id:  q.id,
          time_taken:   t.time_taken,
          option_index: selIdx >= 0 ? selIdx : 0,
          is_correct:   false,   // backend determines correctness
        }
      })

    try {
      // BUG 2 FIX: send ONE flat JSON body with answers + optional timings
      const res = await QuizAPI.submit(id, formattedAnswers, timingsPayload)
      setResult(res)
    } catch (e) {
      if (e.message?.toLowerCase().includes('already')) {
        toast('You have already submitted this quiz.', 'info')
  ...
  }, [id, answers, quiz, current, submitting, navigate, toast])
  ...
  // BUG 3 FIX: isLastQuestion is always based on the full questions array length
      ...
      <div className="quiz-take-body">
        ...
        {q && (
          <div className="question-card">
            ...
            {/* BUG 3 FIX: navigation buttons always correct */}
            <div className="question-nav-btns">
              <button
                className="btn btn-secondary"
                disabled={current === 0}
              ...
              {/* Show Next when NOT on last question, Submit when ON last question */}
              {!isLastQuestion ? (
                <button
                  className="btn btn-primary"
                  onClick={() => handleNavigate(current + 1)}
