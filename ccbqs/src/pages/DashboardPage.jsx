...
export default function DashboardPage() {
  ...
  return (
    ...
    >
      ...
      {/* ── ML: Recommendations (students only) ────────────────────────────── */}
      {!isTeacher && <RecommendationsWidget />}
      ...
      {(loading || upcoming.length > 0) && (
        <div>
          ...
          {loading ? (
            <div className="grid-auto">{[1,2].map(i => <SkeletonCard key={i} />)}</div>
          ) : (
            <div className="grid-auto">
              {upcoming.map(q => <QuizCard key={q.id} quiz={q} />)}
            </div>
      ...
      {!loading && (
        <div style={{ marginTop: 32 }}>
          <h2 style={{ fontSize: 18, marginBottom: 16 }}>ML Features</h2>
          <div className="grid-3">
            ...
            >
              ...
              <h4 style={{ fontSize: 14, marginBottom: 4 }}>AI Insights</h4>
              <p style={{ fontSize: 12, color: 'var(--text-2)' }}>
                Weak topic detection + personalised improvement tips
              </p>
            </div>
            <div
              className="card"
              style={{ padding: 20, cursor: 'pointer', borderColor: 'transparent',
                background: 'linear-gradient(135deg, #e6fff1, #e0fff5)' }}
              onClick={() => navigate('/smart-leaderboard')}
            >
              <div style={{ fontSize: 28, marginBottom: 10 }}>🏆</div>
              <h4 style={{ fontSize: 14, marginBottom: 4 }}>Smart Leaderboard</h4>
              <p style={{ fontSize: 12, color: 'var(--text-2)' }}>
                Ranked by consistency + improvement, not just score
              </p>
            </div>
            <div
              className="card"
              style={{ padding: 20, cursor: 'pointer', borderColor: 'transparent',
                background: 'linear-gradient(135deg, #fff8e6, #fffbe0)' }}
              onClick={() => navigate('/analytics')}
            >
              <div style={{ fontSize: 28, marginBottom: 10 }}>📊</div>
              <h4 style={{ fontSize: 14, marginBottom: 4 }}>Analytics</h4>
              <p style={{ fontSize: 12, color: 'var(--text-2)' }}>
                Score trends and subject-level performance charts
              </p>
            </div>
          </div>
        </div>
