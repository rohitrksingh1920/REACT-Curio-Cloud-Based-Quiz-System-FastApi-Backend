...
// //       msg = data.detail || data.message || `HTTP ${res.status}`
// //     }
// //     if (res.status === 400) msg = msg || 'Bad request.'
// //     if (res.status === 403) msg = msg || 'Access denied.'
// //     if (res.status === 404) msg = msg || 'Resource not found.'
// //     if (res.status === 409) msg = msg || 'Conflict — resource already exists.'
// //     if (res.status === 413) msg = msg || 'File too large.'
...
// //   create: (payload) => apiFetch('/quizzes', { method: 'POST', body: JSON.stringify(payload) }),
// //   update: (id, payload) =>
// //     apiFetch(`/quizzes/${id}`, { method: 'PATCH', body: JSON.stringify(payload) }),
// //   delete: (id) => apiFetch(`/quizzes/${id}`, { method: 'DELETE' }),
// //   take:   (id) => apiFetch(`/quizzes/${id}/take`),
...
// //    * Submit quiz answers.
// //    *
// //    * BUG 2 FIX: timings is now sent in the SAME JSON body as answers.
// //    * Backend SubmitPayload schema has timings as Optional — if null/empty
// //    * it is simply omitted and ML features are skipped gracefully.
// //    *
// //    * @param {number|string} id        - quiz ID
// //    * @param {Array}         answers   - [{ question_id, selected_option_id }]
// //    * @param {Array|null}    timings   - optional: [{ question_id, time_taken, option_index, is_correct }]
// //    */
// //   submit: (id, answers, timings = null) =>
// //     apiFetch(`/quizzes/${id}/submit`, {
// //       method: 'POST',
// //       body: JSON.stringify({
// //         answers,
// //         // Only include timings if non-empty — keeps payload clean
...
// // // ── Enrollment ────────────────────────────────────────────────────────────────
// // export const EnrollAPI = {
// //   enroll: (quizId, user_ids) =>
// //     apiFetch(`/quizzes/${quizId}/enroll`, {
// //       method: 'POST',
// //       body: JSON.stringify({ user_ids }),
// //     }),
// //   removeEnrollment: (quizId, uid) =>
// //     apiFetch(`/quizzes/${quizId}/enroll/${uid}`, { method: 'DELETE' }),
// //   getStudents: (quizId) => apiFetch(`/quizzes/${quizId}/students`),
// // }
...
// //     apiFetch('/settings/profile', { method: 'PATCH', body: JSON.stringify(payload) }),
// //   uploadAvatar:  (formData) =>
// //     apiFetch('/settings/profile/avatar', { method: 'POST', body: formData }),
// //   requestOtp: () =>
// //     apiFetch('/settings/security/request-otp', { method: 'POST' }),
// //   verifyOtp: (otp, new_password, confirm_password) =>
// //     apiFetch('/settings/security/verify-otp', {
// //       method: 'POST',
// //       body: JSON.stringify({ otp, new_password, confirm_password }),
...
// //   toggleActive: (id) =>
// //     apiFetch(`/admin/users/${id}/activate`, { method: 'PATCH' }),
// //   deleteUser:   (id) =>
...
// /**
//  * api/index.js — fixed submit() signature
//  *
//  * FIX: QuizAPI.submit() now accepts optional `timings` param
//  * and sends it in the SAME JSON body as `answers`:
//  *   { answers: [...], timings: [...] }
//  *
//  * Previously submit() only accepted (id, answers), so
//  * TakeQuizPage.jsx's timingsPayload was silently dropped.
//  */
...
//     if (res.status === 403) msg = msg || 'Access denied.'
//     if (res.status === 404) msg = msg || 'Resource not found.'
//     if (res.status === 409) msg = msg || 'Resource already exists.'
//     if (res.status === 413) msg = msg || 'File too large.'
//     if (res.status === 422) msg = msg || 'Validation error. Check your input.'
//     if (res.status === 500) msg = msg || 'Server error. Please try again.'
//     if (res.status === 503) msg = msg || 'Service unavailable.'
//     throw new Error(msg)
//   }
...
//   /**
//    * Submit quiz answers with optional ML timings.
//    *
//    * FIX: timings is now sent in the SAME JSON body as answers.
//    * Backend SubmitPayload has timings as Optional — if absent, ML skipped.
//    *
//    * @param {string|number} id        - quiz ID
//    * @param {Array}         answers   - [{ question_id, selected_option_id }]
//    * @param {Array|null}    timings   - optional ML timings
//    */
//   submit: (id, answers, timings = null) =>
//     apiFetch(`/quizzes/${id}/submit`, {
//       method: 'POST',
...
// export const EnrollAPI = {
//   enroll:          (quizId, user_ids) =>
//     apiFetch(`/quizzes/${quizId}/enroll`, { method: 'POST', body: JSON.stringify({ user_ids }) }),
//   removeEnrollment:(quizId, uid) =>
//     apiFetch(`/quizzes/${quizId}/enroll/${uid}`, { method: 'DELETE' }),
//   getStudents:     (quizId)      => apiFetch(`/quizzes/${quizId}/students`),
// }
