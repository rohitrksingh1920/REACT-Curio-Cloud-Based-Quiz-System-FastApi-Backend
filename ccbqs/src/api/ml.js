...
export const AdaptiveAPI = {
  getNextQuestions: (quizId, answeredIds = []) =>
    apiFetch(`/api/ml/adaptive/${quizId}?answered_ids=${answeredIds.join(',')}`),

  getQuestionDifficulties: (quizId) =>
    apiFetch(`/api/ml/difficulty/${quizId}`),
}

// ── Feature 2: Recommendations ───────────────────────────────────────────────
export const RecommendAPI = {
  get: (limit = 5) => apiFetch(`/api/ml/recommendations?limit=${limit}`),
}

// ── Feature 3: Insights & Weak Topics ────────────────────────────────────────
export const InsightsAPI = {
  get: () => apiFetch('/api/ml/insights'),
}
...
export const CheatingAPI = {
  list: (reviewed, severity) => {
    ...
    if (reviewed !== undefined && reviewed !== null) p.set('reviewed', reviewed)
    if (severity) p.set('severity', severity)
    return apiFetch(`/api/ml/cheating-flags?${p}`)
  },
  review: (id) => apiFetch(`/api/ml/cheating-flags/${id}/review`, { method: 'PATCH' }),
}

// ── Feature 6: Smart Leaderboard ─────────────────────────────────────────────
export const SmartLeaderboardAPI = {
  get: (quizId) => apiFetch(`/api/ml/smart-leaderboard/${quizId}`),
}
